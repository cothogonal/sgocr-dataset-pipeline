...
# ita10 goal:
#   Sweep the groundback IoU threshold to find the yield-vs-quality frontier,
#   and cement ita09_full_push with reliable eval (HTTP retry now in place).
#
#   ita09 post-mortem:
#     Winner: ita09_full_push (IoU=0.50 + leaky_label_hard_reject):
#       sweep=15.5625, vision-nec=51.2%, text-leaky=30.1%, 256 accepted (38.7% rate).
#
#     Eval reliability problem:
#       3 of 4 variants had 100% inline frontier errors due to a transient
#       gemini-3-flash-preview API outage during the sweep.  call_gemini_inline_frontier_batched
#       had no HTTP retry — any 429/503 immediately propagated as a failure.
#       Result: unreliable sweep scores for ita09_gb_leaky_reject, ita09_gb_iou50,
#       ita09_leaky_pure; only ita09_full_push (last variant, outage had cleared) scored.
#
#     Yield problem:
#       IoU=0.50 + leaky_label cuts to 256 rows (38.7% yield, Q3=58.25).
#       IoU=0.35 + leaky_label gave 393 rows (59.9% yield, Q3=67.35) in ita09_gb_leaky_reject
#       but we couldn't eval it.  The leaky_label filter itself removes ~36% of rows
#       when combined with IoU=0.50 but only ~2% when combined with IoU=0.35 (0 leaky
#       candidates in gb_leaky_reject).  Most of the yield loss at IoU=0.50 comes from
#       stricter groundback rejection, not from the leaky_label filter.
#
#     Q3 scores (API-free, reliable):
#       ita09_gb_iou50=72.20 > ita09_leaky_pure=67.42 > ita09_gb_leaky_reject=67.35
#       > ita09_full_push=58.25.  Lower Q3 for full_push reflects low yield/per-image
#       density despite high vision-necessity.
#
#   Engineering change in ita10:
#     dev40_complete.py: _gemini_post_with_http_retry — wraps all Gemini inline frontier
#     POSTs with exponential-backoff retry (up to 4 attempts, 1s/2s/4s waits) on 429
#     and 5xx responses.  This should produce reliable inline eval for all variants.
#
#   Variants (cement × 2, push × 2):
#     ita10_iou35_leaky_cement  — IoU=0.35 + leaky_label_hard_reject [cement]
#       Replicates ita09_gb_leaky_reject.  We couldn't eval this in ita09 — now with
#       retry fix we will get a real vision-nec number for the high-yield config.
#     ita10_fp_cement           — IoU=0.50 + leaky_label_hard_reject [cement]
#       Replicates ita09_full_push.  Confirms winner score is reproducible and that
#       the retry fix doesn't change outcomes when the API is healthy.
#     ita10_iou40_leaky         — IoU=0.40 + leaky_label_hard_reject [push]
#       Middle-ground IoU.  Hypothesis: IoU=0.40 recovers significant yield (close to
#       ita10_iou35) while keeping higher vision-necessity than iou35.
#     ita10_iou45_leaky         — IoU=0.45 + leaky_label_hard_reject [push]
#       Tighter middle-ground.  Hypothesis: yield between iou40 and fp, possibly better
#       Q3 × vision-nec product than either endpoint.
#
#   Key comparisons:
#     ita10_iou35_leaky_cement vs ita09_full_push → is higher yield + leaky_label good
#       enough, or does strict IoU matter?
#     ita10_fp_cement vs ita09_full_push         → reproducibility check
#     ita10_iou40_leaky vs ita10_iou35 + iou50   → find the elbow in yield/quality curve
#     ita10_iou45_leaky vs ita10_iou40_leaky      → does tightening 0.05 matter?
#
#   Cache strategy:
#     All variants use groundback → cache_level="ocr" for all.
#     Cache from ita09_full_push intermediate (OCR detections reused, anchors rebuilt).

LIVE_DOC_DIR = REPO_ROOT / "tasks" / "mm_bridge" / "docs"
LIVE_DOC_NAME = "163_sgocr_mixed_ita10_launch_2026-04-14.md"

_ITA09_FULL_PUSH_INTERMEDIATE = (
    "/home/wdree/percy/vqafromscratch/data/ocr_spatial_qa/intermediate/mixed_dev150"
    "/sgocr_mixed_ita09_20260414_011218_ita09_full_push"
)

VARIANT_ORDER = [
    "ita10_iou35_leaky_cement",
    "ita10_fp_cement",
    "ita10_iou40_leaky",
    "ita10_iou45_leaky",
]
...
VARIANT_DESCRIPTIONS = {
    "ita10_iou35_leaky_cement": "groundback(IoU=0.35) + leaky_label_hard_reject [cement]",
    "ita10_fp_cement":          "groundback(IoU=0.50) + leaky_label_hard_reject [cement, replication of ita09_full_push]",
    "ita10_iou40_leaky":        "groundback(IoU=0.40) + leaky_label_hard_reject [push]",
    "ita10_iou45_leaky":        "groundback(IoU=0.45) + leaky_label_hard_reject [push]",
}

# All ita10 variants change groundback IoU → must rebuild from anchor stage (cache_level="ocr")
_QWEN_MODIFYING: frozenset[str] = frozenset(VARIANT_ORDER)


def parse_args() -> argparse.Namespace:
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ap = argparse.ArgumentParser(
        description="Run ita10: sweep groundback IoU threshold (0.35/0.40/0.45/0.50) + leaky_label."
    )
    ap.add_argument("--bundle-id", default=f"sgocr_mixed_ita10_{stamp}")
    ap.add_argument("--source-name", default="chartqa50_textocr50_cocotext50_source_20260410_110952")
    ap.add_argument("--reference-report-json", default="sgocr_mixed_ita09_20260414_011218_report.json")
    ...
    ap.add_argument(
        ...
        default=_ITA09_FULL_PUSH_INTERMEDIATE,
        help="Intermediate dir from ita09_full_push for OCR cache reuse.",
    )
...
def _render_live_doc(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Goal",
        "",
        "**Sweep groundback IoU threshold (0.35 → 0.50) to find optimal yield-vs-quality balance.**",
        "",
        "ita09 post-mortem:",
        "- Winner: `ita09_full_push` (IoU=0.50 + leaky_label): sweep 15.5625, vision-nec 51.2%,",
        "  text-leaky 30.1%, 256 rows (38.7% accept rate).",
        "- Eval reliability failure: 3/4 variants had 100% inline frontier API errors due to a",
        "  transient `gemini-3-flash-preview` outage. `call_gemini_inline_frontier_batched` had no",
        "  HTTP retry — 429/5xx errors propagated immediately as exceptions.",
        "- Q3 scores (API-free): gb_iou50=72.20 > leaky_pure=67.42 > gb_leaky_reject=67.35",
        "  > full_push=58.25. Low Q3 for full_push from low yield (38.7% vs ~60% for others).",
        "- `ita09_gb_leaky_reject` (IoU=0.35 + leaky, 393 rows, Q3=67.35) had no eval scores.",
        "  Key question: does its vision-nec match or exceed full_push?",
        "",
        "New engineering change: HTTP retry with exponential backoff in `_gemini_post_with_http_retry`.",
        "Wraps all Gemini inline frontier POSTs with up to 4 attempts (waits: 1s/2s/4s) on 429/5xx.",
        "",
        f"Cache: ita09_full_push intermediate (OCR reuse). Workers: {perf['workers']}.",
        "",
        "## Engineering Changes",
        "",
        "| Location | Change |",
        "|---|---|",
        "| `dev40_complete.py` | New `_gemini_post_with_http_retry`: exponential backoff on 429/5xx (4 attempts, 1s/2s/4s) |",
        "",
        "## Variant Summary",
        "",
    ...
    lines += [
        ...
        "",
        "| Variant | IoU | Accepted | Yield% | Q3 | Sweep | Vision-nec% | Text-leaky% |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    iou_map = {
        "ita10_iou35_leaky_cement": 0.35,
        "ita10_fp_cement": 0.50,
        "ita10_iou40_leaky": 0.40,
        "ita10_iou45_leaky": 0.45,
    }
    for name in VARIANT_ORDER:
        result = results.get(name) or {}
        m = result.get("metrics") or {}
        ...
        q3 = m.get("q3_score", 0.0)
        sweep = m.get("sweep_score", 0.0)
        vnec = idep.get("vision_necessary_rate", 0.0) * 100
        tleaky = idep.get("text_leaky_rate", 0.0) * 100
        if accepted > 0:
            lines.append(
                f"| `{name}` | {iou} | {accepted} | {yield_pct:.1f}% | {q3:.2f} | {sweep:.4f} | {vnec:.1f}% | {tleaky:.1f}% |"
            )
        else:
            lines.append(f"| `{name}` | {iou} | — | — | — | — | — | — |")
...
def main() -> None:
    ...
    common_env = {
        **baseline_env,
        ...
        # ita09: leaky_label_hard_reject ON for all ita10 variants
        ...
        "ita10_iou35_leaky_cement": {
            "env": {
                **common_env,
                ...
                "SGOCR_ANCHOR_LABEL_GROUNDBACK_IOU_THRESHOLD": "0.35",
            },
            "note": VARIANT_DESCRIPTIONS["ita10_iou35_leaky_cement"],
        },
        "ita10_fp_cement": {
            "env": {
                **common_env,
                "SGOCR_ANCHOR_LABEL_GROUNDBACK_ENABLED": "1",
                "SGOCR_ANCHOR_LABEL_GROUNDBACK_IOU_THRESHOLD": "0.50",
            },
            "note": VARIANT_DESCRIPTIONS["ita10_fp_cement"],
        },
        "ita10_iou40_leaky": {
            "env": {
                **common_env,
            ...
            "note": VARIANT_DESCRIPTIONS["ita10_iou40_leaky"],
        },
        "ita10_iou45_leaky": {
            "env": {
                **common_env,
            ...
            "note": VARIANT_DESCRIPTIONS["ita10_iou45_leaky"],
        },
    }
    ...
    for name, spec in variants.items():
        ...
        # All ita10 variants have groundback enabled → must rebuild from OCR stage.
        ...
        if result.get("status") in {"ok", "cached"}:
            ...
            if not args.skip_image_dependence and rows:
                ...
                _write_all(payload=payload, report_json=report_json, report_md=report_md, live_doc=live_doc)
                try:
                    idep_dir = out_dir / "evals" / "image_dependence_ita10"
