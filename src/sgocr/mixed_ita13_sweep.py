...
# ita13 goal:
#   Fix ita12 rgboost TP/AP collapse while recovering vision-nec% without the
#   paid frontier gate. Two root causes to fix:
#
#   ita12 post-mortem:
#     ita12_full:           accepted=289, sweep=12.54, vn=46.7%, tl=41.4%, TP=72, AP=32, RG=17
#     ita12_offset20:       accepted=275, sweep=12.23, vn=48.2%, tl=39.6%, TP=68, AP=29, RG=16
#     ita12_rgboost:        accepted=271, sweep=12.08, vn=44.3%, tl=43.1%, TP=25, AP=11, RG=46
#     ita12_rgboost_offset20: accepted=261, sweep=11.76, vn=45.9%, tl=41.7%, TP=22, AP=10, RG=44
#
#   Problem 1 — TP/AP collapse under boost:
#     With target=4 and caps each at 1 for RG/TP/AP, slots fill as: 2×DR + 1×YN + 1×RG.
#     The RG boost (1.5) outcompetes TP/AP for the 1 property slot. In rgboost variants,
#     TP dropped from 72→25 and AP from 32→11.
#     Fix: raise target_per_image 4→5 + rg_per_image_hard_cap=2. At target=5 the typical
#     fill is 2×DR + 1×YN + 1×RG + 1×TP/AP. The 2nd RG cap allows a 2nd RG slot to exist
#     without stealing the property slot. RG boost=1.5 ensures the 2nd RG slot is chosen
#     over a 3rd DR.
#
#   Problem 2 — Low vision-nec% without frontier gate:
#     ita11 (with gate) achieved 55.9% vn; ita12_full (no gate) dropped to 46.7%.
#     The frontier gate was implicitly filtering text-answerable rows (high false positives
#     on easy DR questions). TP/AP types have 87-100% per-type image accuracy — they are
#     the strongest vision-dep signal. Adding property_candidate_selection_bonus boosts
#     TP/AP into more slots, recovering vision-nec% organically without the gate.
#
#   Engineering changes (all in semantic_dev40_tuning.py + dev40_complete.py):
#     - rg_per_image_hard_cap (SGOCR_RG_PER_IMAGE_HARD_CAP=2): configurable RG cap in
#       enforce_type_constraints (TP/AP remain capped at 1)
#     - property_candidate_selection_bonus (SGOCR_PROPERTY_CANDIDATE_SELECTION_BONUS):
#       score bonus for TEXT_PROPERTY/ANCHOR_PROPERTY in select_candidates
#
#   Variants (all: no frontier gate, target=5, hard_cap=2, rg_boost=1.5, IoU=0.40, leaky):
#     ita13_base          — target=5, cap=2, boost=1.5, offset=0.15, no property bonus [base]
#     ita13_propbonus     — same + property_bonus=0.5 [vision-nec push]
#     ita13_offset20      — ita13_base + centroid_offset=0.20 [spatial push]
#     ita13_propbonus_offset20 — property_bonus=0.5 + offset=0.20 [max quality push]
#
#   Cache strategy:
#     run1 (ita13_base):           cache_level="verified" from ita12_full. Qwen skipped.
#                                  Clean baseline — no "with" fix, post-verified ops only.
#     run2 (ita13_propbonus):      cache_level="ocr" from ita12_full. Qwen re-runs fresh,
#                                  exercising the "with" connector fix in
#                                  QWEN_INDEPENDENT_RAW_INVENTORY_CATEGORY_TEXT.
#     runs 3+4:                    cache_level="verified" from run2's intermediate.
#                                  Qwen skipped — verified_tuples came from run2's fresh
#                                  Qwen pass, so "with" labels are inherited without a
#                                  second GPU run.
#
#   Expected outcomes:
#     ita13_base: ~300+ rows, 30+ RG, TP/AP recovered (TP~65+, AP~25+), vn~48%
#     ita13_propbonus: fewer DR, more TP/AP, vn~52%+
#     ita13_offset20: slightly fewer rows, text-leaky < 38%
#     ita13_propbonus_offset20: fewest rows, highest vn target (54%+)

LIVE_DOC_DIR = REPO_ROOT / "tasks" / "mm_bridge" / "docs"
LIVE_DOC_NAME = "167_sgocr_mixed_ita13_launch_2026-04-14.md"

_ITA12_FULL_INTERMEDIATE = (
    "/home/wdree/percy/vqafromscratch/data/ocr_spatial_qa/intermediate/mixed_dev150"
    "/sgocr_mixed_ita12_20260414_105342_ita12_full"
)
...
VARIANT_ORDER = [
    ...
    "ita13_propbonus",
    "ita13_offset20",
    "ita13_propbonus_offset20",
]

VARIANT_DESCRIPTIONS = {
    "ita13_base":               "No gate, target=5, cap=2, boost=1.5, offset=0.15, no prop bonus [base, verified cache]",
    "ita13_propbonus":          "No gate, target=5, cap=2, boost=1.5, offset=0.15, prop_bonus=0.5, qwen-with [ocr cache — Qwen re-run]",
    "ita13_offset20":           "No gate, target=5, cap=2, boost=1.5, offset=0.20, no prop bonus, qwen-with [verified from propbonus]",
    "ita13_propbonus_offset20": "No gate, target=5, cap=2, boost=1.5, offset=0.20, prop_bonus=0.5, qwen-with [verified from propbonus]",
}
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="Run ita13: fix ita12 TP/AP collapse + recover vision-nec% via property bonus."
    )
    ap.add_argument("--bundle-id", default=f"sgocr_mixed_ita13_{stamp}")
    ap.add_argument("--source-name", default="chartqa50_textocr50_cocotext50_source_20260410_110952")
    ap.add_argument("--reference-report-json", default="sgocr_mixed_ita12_20260414_105342_report.json")
    ...
    ap.add_argument("--rg-per-image-hard-cap", type=int, default=2)
...
def _render_live_doc(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Goal",
        "",
        "**Fix ita12 TP/AP collapse + recover vision-nec% without the paid frontier gate.**",
        "",
        "ita12 post-mortem:",
        "- ita12_full: 289 rows, vn=46.7%, tl=41.4%, TP=72, AP=32, RG=17.",
        "- ita12_rgboost: 271 rows, vn=44.3%, tl=43.1%, TP=25, AP=11, RG=46.",
        "  RG boost collapsed TP/AP — boost competed for the single shared property slot.",
        "- Root cause: target=4 with caps=1 each for RG/TP/AP → full = 2DR+1YN+1RG, no room for TP/AP.",
        "",
        "Engineering changes:",
        "- `semantic_dev40_tuning.py`: added `rg_per_image_hard_cap` (SGOCR_RG_PER_IMAGE_HARD_CAP)",
        "  and `property_candidate_selection_bonus` (SGOCR_PROPERTY_CANDIDATE_SELECTION_BONUS)",
        "- `dev40_complete.py`: `enforce_type_constraints()` uses configurable RG cap;",
        "  `select_candidates()` applies property bonus to TEXT_PROPERTY/ANCHOR_PROPERTY",
        "- `semantic_grounding.py`: Qwen 'with' connector instruction added to",
        "  `QWEN_INDEPENDENT_RAW_INVENTORY_CATEGORY_TEXT` — exercised in runs 2–4",
        "",
        "Cache strategy: run 1 = verified from ita12_full (baseline, no Qwen re-run).",
        "Run 2 = ocr from ita12_full (Qwen re-runs with 'with' fix, generates new intermediate).",
        f"Runs 3+4 = verified from run 2 intermediate (inherit 'with' labels). Workers: {perf['workers']}.",
        "",
        "## Engineering Changes",
        "",
        "| Location | Change |",
        "|---|---|",
        "| `semantic_dev40_tuning.py` | Added `rg_per_image_hard_cap` + `SGOCR_RG_PER_IMAGE_HARD_CAP` env-var |",
        "| `semantic_dev40_tuning.py` | Added `property_candidate_selection_bonus` + `SGOCR_PROPERTY_CANDIDATE_SELECTION_BONUS` env-var |",
        "| `dev40_complete.py` | `enforce_type_constraints()` uses `rg_per_image_hard_cap` (TP/AP remain capped at 1) |",
        "| `dev40_complete.py` | `select_candidates()` applies `property_candidate_selection_bonus` to TP/AP candidates |",
        "| `semantic_grounding.py` | Qwen 'with' connector added to `QWEN_INDEPENDENT_RAW_INVENTORY_CATEGORY_TEXT` (deferred) |",
        "",
        "## Variant Summary",
        "",
    ...
    lines += [
        ...
        "",
        "| Variant | DR | RG | YN | TP | AP | DR% | RG% | TP% | AP% |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for name in VARIANT_ORDER:
        result = results.get(name) or {}
        m = result.get("metrics") or {}
        qt = m.get("question_types") or {}
        total = m.get("accepted_qas", 0)
        if not total:
            lines.append(f"| `{name}` | — | — | — | — | — | — | — | — | — |")
            continue
        dr = qt.get("DIRECT_READ", 0)
        rg = qt.get("REVERSE_GROUND", 0)
        yn = qt.get("YES_NO", 0)
        tp = qt.get("TEXT_PROPERTY", 0)
        ap = qt.get("ANCHOR_PROPERTY", 0)
        lines.append(
            f"| `{name}` |"
            f" {dr} | {rg} | {yn} | {tp} | {ap} |"
            f" {dr/total*100:.1f}% | {rg/total*100:.1f}% |"
            f" {tp/total*100:.1f}% | {ap/total*100:.1f}% |"
        )
    ...
    lines += [
        ...
        "",
        "| Variant | Offset | Target | Cap | Boost | PropBonus | Accepted | Sweep | Vision-nec% | Text-leaky% |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    offset_map = {
        "ita13_base":               0.15,
        "ita13_propbonus":          0.15,
        "ita13_offset20":           0.20,
        "ita13_propbonus_offset20": 0.20,
    }
    bonus_map = {
        "ita13_base":               0.0,
        "ita13_propbonus":          0.5,
        "ita13_offset20":           0.0,
        "ita13_propbonus_offset20": 0.5,
    }
    for name in VARIANT_ORDER:
        result = results.get(name) or {}
        m = result.get("metrics") or {}
        ...
        if accepted > 0:
            lines.append(
                f"| `{name}` | {offset} | 5 | 2 | 1.5 | {bonus} | {accepted} | {sweep:.4f} | {vnec:.1f}% | {tleaky:.1f}% |"
            )
        else:
            lines.append(f"| `{name}` | {offset} | 5 | 2 | 1.5 | {bonus} | — | — | — | — |")
...
def main() -> None:
    ...
    common_env = {
        **baseline_env,
        ...
        # ita08 typed gate: intentionally NOT set (no paid gate in ita13)
        ...
        # ita12: RG candidate oversampling boost
        "SGOCR_RG_CANDIDATE_OVERSAMPLE_BOOST": str(float(args.rg_candidate_oversample_boost)),
        # ita13: configurable RG hard cap (TP/AP remain capped at 1)
        "SGOCR_RG_PER_IMAGE_HARD_CAP": str(int(args.rg_per_image_hard_cap)),
    }
    ...
    # Property candidate selection bonus (TEXT_PROPERTY + ANCHOR_PROPERTY)
    prop_bonus_flags = {
    ...
    common_cli = {
        **baseline_cli,
        "--target-per-image": str(int(args.target_per_image)),
    }

    # run1: verified cache from ita12_full (no Qwen re-run, no "with" fix — clean baseline)
    _ita12_full_cache = Path(_ITA12_FULL_INTERMEDIATE)

    # run2: ocr cache from ita12_full (Qwen re-runs, exercising the "with" connector fix)
    # runs 3+4: verified cache from run2's intermediate (inherit run2's "with" Qwen labels)
    _ita13_propbonus_intermediate = root_intermediate / f"{args.bundle_id}_ita13_propbonus"
    ...
    variants: dict[str, dict[str, Any]] = {
        "ita13_base": {
            ...
            "cli": dict(common_cli),
            "note": VARIANT_DESCRIPTIONS["ita13_base"],
            "cache_dir": _ita12_full_cache,
        ...
        "ita13_propbonus": {
            ...
            "cli": dict(common_cli),
            "note": VARIANT_DESCRIPTIONS["ita13_propbonus"],
            "cache_dir": _ita12_full_cache,
            "cache_level": "ocr",
        },
        "ita13_offset20": {
            ...
            "cli": dict(common_cli),
            "note": VARIANT_DESCRIPTIONS["ita13_offset20"],
            "cache_dir": _ita13_propbonus_intermediate,
            "cache_level": "verified",
        },
        "ita13_propbonus_offset20": {
            "env": {
            ...
            "cli": dict(common_cli),
            "note": VARIANT_DESCRIPTIONS["ita13_propbonus_offset20"],
            "cache_dir": _ita13_propbonus_intermediate,
            "cache_level": "verified",
        },
    }
    ...
    if existing_payload:
        ...
        payload = {
            ...
            "performance_profile": {
                ...
                "rg_per_image_hard_cap": int(args.rg_per_image_hard_cap),
    ...
    for name, spec in variants.items():
        ...
        if result.get("status") in {"ok", "cached"}:
            ...
            if not args.skip_image_dependence and rows:
                ...
                _write_all(payload=payload, report_json=report_json, report_md=report_md, live_doc=live_doc)
                try:
                    idep_dir = out_dir / "evals" / "image_dependence_ita13"
