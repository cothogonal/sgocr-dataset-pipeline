...
# Dev200 source: 200 images from the full manifest
SOURCE_EXPERIMENT = OCR_SPATIAL_QA_FINAL_ROOT / "dev200" / "dev200_source_universe"

# Dev40 reference runs for the report
C06_DEV40 = OCR_SPATIAL_QA_FINAL_ROOT / "dev200" / "sgocr_dev40_baseline_20260406_180505_c06_varied_wording"
D03_DEV40 = OCR_SPATIAL_QA_FINAL_ROOT / "dev200" / "sgocr_dev40_prefinal_20260406_193347_d03_c06_22tags"


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Run the 3-run dev200 scale sweep.")
    ap.add_argument("--bundle-id", default=f"sgocr_dev200_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
def build_specs(bundle_id: str) -> list[AblationSpec]:
    # -----------------------------------------------------------------
    # Shared base: the proven c06 architecture.
    # Varied wording + centroid spatial + detect 0.35 + bigcrop 0.10
    # + merge + ambig 7/6 + Flash teacher.
    # -----------------------------------------------------------------
    base_env = {
        # Merge stage
    ...
    # e01 runs full pipeline; e02-e03 reuse its detection+OCR
    e01_intermediate = OCR_SPATIAL_QA_INTERMEDIATE_ROOT / "dev200" / f"{bundle_id}_e01_c06_dev200"

    return [
        # =============================================================
        # e01: c06 architecture at dev200 scale
        #   The proven dev40 winner, unchanged, on 200 images.
        #   14 tags, grounding 0.28.  cache_level="none" — full run.
        # =============================================================
        AblationSpec(
            "e01_c06_dev200",
            "c06 winner architecture (varied wording, extreme local, 14 tags) on dev200",
            "none",
            cli={
        ...
        # =============================================================
        # e02: d03 architecture at dev200 scale
        #   22 tags + lower grounding threshold for max anchor coverage.
        #   Reuses e01 detection+OCR.
        # =============================================================
        AblationSpec(
            "e02_d03_dev200",
            "d03 architecture (varied wording, extreme local, 22 tags, grounding 0.22) on dev200",
            "ocr",
            cli={
            ...
            env=dict(base_env),
            cache_intermediate_dir=e01_intermediate,
        ),

        # =============================================================
        # e03: WILDCARD — "mixed hunter"
        #   The one thing missing is rev-mixed.  This run cranks mixed
        #   bias to 0.70/0.45, softens the reverse ambiguity gate to 8,
        #   uses 22 tags, and bumps target-per-image to 7 for more
        #   candidate diversity.  More candidates × higher mixed
        #   probability × softer gate = max mixed chance.
        #   Also uses aggressive anchor expansion to give more anchors
        #   clean local metadata (a prerequisite for local/mixed scope).
        # =============================================================
        AblationSpec(
            "e03_mixed_hunter_dev200",
            "Wildcard: extreme mixed bias (0.70/0.45), softer reverse gate (8), 22 tags, 7 per image, aggressive anchors",
            "ocr",
            cli={
                "--grounding-threshold": "0.22",
                "--max-tags-per-image": "22",
            ...
            env={
                **base_env,
                # Crank mixed bias way up
                "SGOCR_REVERSE_GROUND_DIRECTIONAL_LOCAL_BIAS": "0.55",
                "SGOCR_REVERSE_GROUND_ON_LOCAL_BIAS": "0.30",
                "SGOCR_REVERSE_GROUND_DIRECTIONAL_MIXED_BIAS": "0.70",
                "SGOCR_REVERSE_GROUND_ON_MIXED_BIAS": "0.45",
                # Soften reverse ambiguity gate — let more mixed answers through
                "SGOCR_AMBIGUITY_REVERSE_REJECT_SCORE": "8",
                # Aggressive anchor expansion — more anchors with clean local metadata
...
def render_report(bundle_id: str, specs: list[AblationSpec], results: dict[str, dict[str, Any]], docs_path: Path) -> str:
    lines = [
        "# SGOCR Dev200 Scale Sweep",
        "",
        f"- Bundle: `{bundle_id}`",
        f"- Image universe: 200 images (full dev200 manifest)",
        f"- Updated: `{now_stamp()}`",
        "",
        "## Design",
        "",
        "3-run sweep scaling the proven dev40 architectures to 200 images.",
        "e01 = c06 winner (14 tags), e02 = d03 (22 tags), e03 = wildcard mixed hunter.",
        "",
        "## Dev40 Reference (for comparison)",
        "",
        "| Run | Accepted | Accept% | Images/40 | Q3 | Rev local | Rev mixed |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for name, path in (("c06_dev40", C06_DEV40), ("d03_dev40", D03_DEV40)):
        try:
            m = collect_metrics(path)
            lines.append(f"| `{name}` | {m['accepted_qas']} | {m['qa_accept_rate']:.3f} | {m['images_with_final_rows']}/40 | {m['q3_score']} | {m['reverse_local']} | {m['reverse_mixed']} |")
        except Exception:
            lines.append(f"| `{name}` | - | - | - | - | - | - |")
    lines.extend([
        "",
        "## Dev200 Scoreboard",
        "",
        "| Run | Status | Accepted | Accept% | Images/200 | Q3 | Neg | Rev local | Rev mixed | Ambig high | Anchor miss | Diversity |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ])
    for spec in specs:
    ...
    lines.extend([
        ...
        "- e01 runs full pipeline (cache_level=none); e02-e03 reuse e01 detection+OCR (cache_level=ocr).",
        "- Q3 coverage component normalizes to 40 images (dev40 formula). For dev200 comparison, look at images/200 and accept rate directly.",
...
def run_ablation(
    *,
    ...
    env = os.environ.copy()
    env.update(spec.env)
    append_timeline(timeline_path, f"START {spec.name} cache={spec.cache_level} model={model_name} images=200 tags={spec.cli.get('--max-tags-per-image','?')}")
...
def main() -> None:
    ...
    docs_path = REPO_ROOT / "tasks" / "mm_bridge" / "docs" / "121_sgocr_dev200_sweep_2026-04-06.md"
    results: dict[str, dict[str, Any]] = {}
    append_timeline(
        timeline_path,
        f"BUNDLE {bundle_id} start specs={len(specs)} images=200",
    )
    write_text(report_path, render_report(bundle_id, specs, results, docs_path))
    write_text(docs_path, render_report(bundle_id, specs, results, docs_path))
    for spec in specs:
