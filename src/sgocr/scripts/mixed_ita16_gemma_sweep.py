...
# ita16 goal: first Gemma4-ollama comparison sweep against ita15 (Qwen).
#
#   Anchor backend: gemma4_ollama (GemmaOllamaAnchorGrounder)
#   OCR cache:      reuse nemotron_v2 text_detections from ita15_t8 intermediate
#   All 4 variants run Gemma; prompt and cap settings vary across variants:
#
#   g_t5_base   — ita15 Gemma prompt + t5 caps: regression/floor baseline
#   g_t8_base   — ita15 Gemma prompt + t8 caps: main Gemma baseline
#   g_t8_antidoc — ita15 Gemma prompt + anti-document label rule + t8 caps
#                  Targets the 'document'×4 mislabel seen in smoke tests
#   g_t8_compact — minimal compact prompt + t8 caps
#                  Tests whether brevity beats the complex multi-rule prompt

VARIANT_ORDER = ["g_t5_base", "g_t8_base", "g_t8_antidoc", "g_t8_compact"]

VARIANT_DESCRIPTIONS: dict[str, str] = {
    "g_t5_base": (
        "Gemma4-ollama + ita15 prompt + target=5 + rg_cap=2 (floor baseline)"
    ),
    "g_t8_base": (
        "Gemma4-ollama + ita15 prompt + target=8 + rg_cap=3 (main Gemma baseline)"
    ),
    "g_t8_antidoc": (
        "Gemma4-ollama + anti-doc prompt + target=8 + rg_cap=3 (forbids 'document' label)"
    ),
    "g_t8_compact": (
        "Gemma4-ollama + compact minimal prompt + target=8 + rg_cap=3 (brevity test)"
    ),
}
...
LIVE_DOC_NAME = "170_sgocr_mixed_ita16_gemma_launch_2026-04-16.md"
...
def _render_sweep_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Goal",
        "",
        "**ita16: first Gemma4-ollama comparison sweep. Four variants across prompt style and cap settings.**",
        "",
        "Changes from ita15:",
        "- `SGOCR_ANCHOR_CANDIDATE_BACKEND=gemma4_ollama`: replaces Qwen3-VL (vLLM) with Gemma4 (Ollama).",
        "- OCR cache reused from ita15_t8 (nemotron_v2 text_detections unchanged).",
        "- Prompt variants: ita15 base, anti-doc, compact (see variant table).",
        "",
        "## Gemma4 Backend",
        "",
        "| Setting | Value |",
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="ita16: Gemma4-ollama anchor comparison sweep (4 variants)."
    )
    ap.add_argument("--bundle-id", default=f"sgocr_mixed_ita16_{stamp}")
    ap.add_argument("--source-name", default="chartqa50_textocr50_cocotext50_source_20260410_110952")
    ap.add_argument("--reference-report-json", default="sgocr_mixed_ita15_20260415_204008_report.json")
...
def main() -> None:
    ...
    # All Gemma variants share this base env — same post-OCR settings as ita15_t8.
    common_env = {
        **baseline_env,
        "SGOCR_OCR_FRONTEND": "nemotron_v2",
        "SGOCR_ANCHOR_TAG_DISCOVERY_BACKEND": "florence",
        # Gemma4 ollama replaces Qwen vLLM
        ...
        # ita15 Gemma prompt enabled by default; variants may override
    ...
    variants: dict[str, dict[str, Any]] = {
        "g_t5_base": {
            ...
            "cli": {**common_cli, "--target-per-image": "5"},
            "note": VARIANT_DESCRIPTIONS["g_t5_base"],
        ...
        "g_t8_base": {
            ...
            "note": VARIANT_DESCRIPTIONS["g_t8_base"],
            "cache_dir": _ita15_t8_cache,
            "cache_level": "ocr",
        },
        "g_t8_antidoc": {
            "env": {
                **common_env,
                "SGOCR_RG_PER_IMAGE_HARD_CAP": "3",
                # antidoc is injected via SGOCR_GEMMA_ITA16_PROMPT_VARIANT
                "SGOCR_GEMMA_ITA16_PROMPT_VARIANT": "antidoc",
            },
            "cli": {**common_cli, "--target-per-image": "8"},
            "note": VARIANT_DESCRIPTIONS["g_t8_antidoc"],
            "cache_dir": _ita15_t8_cache,
            "cache_level": "ocr",
        },
        "g_t8_compact": {
            ...
            "cli": {**common_cli, "--target-per-image": "8"},
            "note": VARIANT_DESCRIPTIONS["g_t8_compact"],
            "cache_dir": _ita15_t8_cache,
            "cache_level": "ocr",
        },
    }
    ...
    for name, spec in variants.items():
        ...
        if result.get("status") in {"ok", "cached"}:
            ...
            if not args.skip_image_dependence and rows:
                ...
                _write_all(payload=payload, report_json=report_json, report_md=report_md, live_doc=live_doc)
                try:
                    idep_dir = out_dir / "evals" / "image_dependence_ita16"
