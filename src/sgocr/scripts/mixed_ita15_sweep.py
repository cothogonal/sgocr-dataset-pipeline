...
# ita15 goal:
#   Stem from ita14 (no_text_ref OCR cache). Two prompt upgrades over ita14:
#
#   1. Qwen ita15 prompt (SGOCR_QWEN_ITA15_PROMPT_ENABLED=1):
#      - Medium-to-large object preference: skips very small text fragments and
#        whole-image backgrounds that clog the anchor pool.
#      - Chart fluff suppression: explicitly ignores copyright notices, watermarks,
#        and decorative side panels; focuses on bars, data points, axes, and titles.
#      - Athlete label fix: photo subjects labeled by role+appearance (e.g.
#        "baseball player with red jersey") to prevent "document" mislabeling.
#      - Prompt compressed to reduce token count and stay within local Qwen limits.
#
#   2. Two-variant cap test:
#      - ita15_t5: reverted caps (target=5, rg_cap=2) — baseline for comparison
#      - ita15_t8: relaxed caps (target=8, rg_cap=3) — tests whether improved
#        prompt specificity fixes the reverse_ground_ambiguous spike seen in ita14
#
#   Cache strategy: OCR cache from ita14 intermediate
#   (reuses text_detections, re-runs Qwen with ita15 prompt).

VARIANT_ORDER = ["ita15_t5", "ita15_t8"]

VARIANT_DESCRIPTIONS: dict[str, str] = {
    "ita15_t5": (
        "ita14 OCR cache + ita15 prompt + target=5 + rg_cap=2 (reverted caps)"
    ),
    "ita15_t8": (
        "ita14 OCR cache + ita15 prompt + target=8 + rg_cap=3 (tests whether ita15 prompt fixes ambiguity spike)"
    ),
}

# OCR cache: reuse text_detections from ita14_no_text_ref
_ITA14_INTERMEDIATE = (
    str(OCR_SPATIAL_QA_INTERMEDIATE_ROOT / "mixed_dev150" /
        "sgocr_mixed_ita14_20260415_193754_ita14_no_text_ref")
)

LIVE_DOC_DIR = REPO_ROOT / "tasks" / "mm_bridge" / "docs"
LIVE_DOC_NAME = "169_sgocr_mixed_ita15_launch_2026-04-15.md"
...
def _render_sweep_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Goal",
        "",
        "**ita15: improved Qwen prompt + two-variant cap test, stemming from ita14 OCR cache.**",
        "",
        "Changes from ita14:",
        "- `SGOCR_QWEN_ITA15_PROMPT_ENABLED=1`: medium-object preference, chart-fluff suppression,",
        "  athlete label fix, compressed prompt.",
        "- `ita15_t5`: reverted caps (target=5, rg_cap=2) — regression baseline.",
        "- `ita15_t8`: relaxed caps (target=8, rg_cap=3) — tests whether better prompt specificity",
        "  fixes the reverse_ground_ambiguous spike (+9.8pp) seen in ita14.",
        "",
        "## Engineering Changes",
        "",
        "| Location | Change |",
        "|---|---|",
        "| `qwen_anchor_vllm.py` | Added `INDEPENDENT_QWEN_INVENTORY_PROMPT_ITA15`, `STRUCTURAL_FALLBACK_QWEN_PROMPT_ITA15`; added athlete/document labels to `_DEGENERATE_ANCHOR_LABELS` |",
        "| `semantic_dev40_tuning.py` | Added `qwen_ita15_prompt_enabled` + `SGOCR_QWEN_ITA15_PROMPT_ENABLED` env-var |",
        "| `full_pipeline_dev40.py` | Wired ita15 prompt (takes priority over no_text_ref) in inventory + structural fallback paths |",
        "",
        "## Variant Summary",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="ita15: ita15 Qwen prompt + two-variant cap test (t5/t8), stemming from ita14 OCR cache."
    )
    ap.add_argument("--bundle-id", default=f"sgocr_mixed_ita15_{stamp}")
    ap.add_argument("--source-name", default="chartqa50_textocr50_cocotext50_source_20260410_110952")
    ap.add_argument("--reference-report-json", default="sgocr_mixed_ita14_20260415_193754_report.json")
...
def main() -> None:
    ...
    common_env = {
        **baseline_env,
        ...
        # ita15 prompt supersedes no_text_ref and anti_ocr
        "SGOCR_QWEN_ITA15_PROMPT_ENABLED": "1",
        # SAM3 not installed in this environment
    ...
    _ita14_cache = Path(_ITA14_INTERMEDIATE)

    variants: dict[str, dict[str, Any]] = {
        "ita15_t5": {
            "env": {
                **common_env,
                "SGOCR_RG_PER_IMAGE_HARD_CAP": "2",
            },
            "cli": {
                **common_cli,
                "--target-per-image": "5",
            },
            "note": VARIANT_DESCRIPTIONS["ita15_t5"],
            "cache_dir": _ita14_cache,
            "cache_level": "ocr",   # reuse text_detections, re-run Qwen with ita15 prompt
        },
        "ita15_t8": {
            "env": {
                **common_env,
            ...
            "note": VARIANT_DESCRIPTIONS["ita15_t8"],
            "cache_dir": _ita14_cache,
            "cache_level": "ocr",   # reuse text_detections, re-run Qwen with ita15 prompt
        },
    }
    ...
    for name, spec in variants.items():
        ...
        if result.get("status") in {"ok", "cached"}:
            ...
            if not args.skip_image_dependence and rows:
                ...
                _write_all(payload=payload, report_json=report_json, report_md=report_md, live_doc=live_doc)
                try:
                    idep_dir = out_dir / "evals" / "image_dependence_ita15"
