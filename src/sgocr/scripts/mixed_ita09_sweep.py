...
# ita09 goal:
#   Cement the ita08_groundback frontier and reduce RG text-leakage via a structural
#   anchor-label filter that does not rely on external API calls.
#
#   ita08 post-mortem:
#     Winner: ita08_groundback (sweep 15.34, vision-nec 49.8%, text-leaky 31.6%).
#
#     Problem 1 — RG vdep check never fired:
#       The configured model ("openai:gpt-5.3-codex") does not exist — all calls raised
#       exceptions, which were caught and defaulted to conservative keep. Result: Corrected=0
#       for every variant, and RG rows with text-solvable answers passed unchecked.
#       Evidence: RG text-only accuracy beat image accuracy in ita08_correction (Δ=−0.14).
#
#     Problem 2 — ita08_full inline frontier API errors (383/397 rows errored):
#       Combined correction+groundback pipeline triggered Gemini rate-limits or timeouts
#       during the inline frontier eval phase. Error-passthrough inflated accept count to 397
#       (vs ~253 for single-gate variants) and zeroed all image-dependence metrics.
#
#     Root cause of RG leakage: anchor_label contains color/shape tokens ("blue rectangular
#     bar", "red circular badge") → these tokens expose the element's visual identity in the
#     question text, making the answer guessable from text alone. 11–14 such rows per variant.
#
#   Engineering change in ita09:
#     New flag: SGOCR_RG_LEAKY_LABEL_HARD_REJECT_ENABLED
#     When set, structurally rejects REVERSE_GROUND rows where anchor_label contains a color
#     or shape token. Zero API cost, zero error risk. Directly removes the identified leakage
#     source that the broken vdep check failed to catch.
#
#   Variants (cement × 2, push × 2):
#     ita09_gb_leaky_reject  — groundback(IoU=0.35) + leaky_label_hard_reject  [cement]
#       Confirms groundback direction works while adding the new structural RG filter.
#     ita09_gb_iou50         — groundback(IoU=0.50) only                        [cement]
#       Isolates effect of tightening groundback threshold vs ita08_groundback baseline.
#     ita09_leaky_pure       — typed_gate (no groundback) + leaky_label_hard_reject [push]
#       Tests if the new RG filter alone (without groundback) can match groundback sweep score.
#       Uses verified cache — cheaper and faster.
#     ita09_full_push        — groundback(IoU=0.50) + leaky_label_hard_reject    [push]
#       Maximum quality stack: tightest groundback + RG structural filter combined.
#
#   Cache strategy:
#     All variants cache from ita08_typed_gate intermediate (OCR + anchor + verified tuples).
#     Groundback variants (ita09_gb_*) → cache_level="ocr" (must rebuild anchor/verified stage).
#     Non-groundback variants → cache_level="verified" (reuse anchors + verified tuples).
#
#   Key comparison axes:
#     ita09_gb_leaky_reject vs ita08_groundback → value of adding structural RG filter
#     ita09_gb_iou50 vs ita08_groundback        → effect of tighter IoU threshold
#     ita09_leaky_pure vs ita08_typed_gate      → leaky_reject impact without groundback
#     ita09_full_push vs ita09_gb_iou50         → additive value of leaky_reject on strict groundback

LIVE_DOC_DIR = REPO_ROOT / "tasks" / "mm_bridge" / "docs"
LIVE_DOC_NAME = "162_sgocr_mixed_ita09_launch_2026-04-14.md"

_ITA08_TYPED_GATE_INTERMEDIATE = (
    "/home/wdree/percy/vqafromscratch/data/ocr_spatial_qa/intermediate/mixed_dev150"
    "/sgocr_mixed_ita08_20260413_180618_ita08_typed_gate"
)

VARIANT_ORDER = [
    "ita09_gb_leaky_reject",
    "ita09_gb_iou50",
    "ita09_leaky_pure",
    "ita09_full_push",
]
...
VARIANT_DESCRIPTIONS = {
    "ita09_gb_leaky_reject": "groundback(IoU=0.35) + leaky_label_hard_reject [cement]",
    "ita09_gb_iou50":        "groundback(IoU=0.50) only [cement]",
    "ita09_leaky_pure":      "typed_gate + leaky_label_hard_reject, no groundback [push]",
    "ita09_full_push":       "groundback(IoU=0.50) + leaky_label_hard_reject [push]",
}

_QWEN_MODIFYING: frozenset[str] = frozenset()  # no variants change Qwen prompt in ita09


def parse_args() -> argparse.Namespace:
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ap = argparse.ArgumentParser(
        description="Run ita09: cement groundback frontier + structural RG leaky-label filter."
    )
    ap.add_argument("--bundle-id", default=f"sgocr_mixed_ita09_{stamp}")
    ap.add_argument("--source-name", default="chartqa50_textocr50_cocotext50_source_20260410_110952")
    ap.add_argument("--reference-report-json", default="sgocr_mixed_ita08_20260413_180618_report.json")
    ...
    ap.add_argument("--groundback-iou-base", type=float, default=0.35,
                    help="IoU threshold for ita09_gb_leaky_reject variant.")
    ap.add_argument("--groundback-iou-strict", type=float, default=0.50,
                    help="Stricter IoU threshold for ita09_gb_iou50 and ita09_full_push.")
    ...
    ap.add_argument(
        ...
        default=_ITA08_TYPED_GATE_INTERMEDIATE,
        help="Intermediate dir from ita08_typed_gate for cache reuse.",
    )
...
def _rg_leakage_stats(rows: list[dict[str, Any]]) -> dict[str, Any]:
    """Diagnostics for RG text-leakage indicators in ita09."""
...
def _render_live_doc(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Goal",
        "",
        "**Cement the ita08_groundback frontier + structural RG leaky-label hard reject.**",
        "",
        "ita08 post-mortem:",
        "- Winner: `ita08_groundback` (sweep 15.34, vision-nec 49.8%, text-leaky 31.6%).",
        "- RG vdep check never fired: `openai:gpt-5.3-codex` is an invalid model name →",
        "  all calls raised exceptions → conservative keep → Corrected=0 for all variants.",
        "- RG text-only accuracy BEAT image accuracy in ita08_correction (Δ=−0.14),",
        "  confirming 11–14 leaky-label RG rows per variant are contaminating the dataset.",
        "- ita08_full had 383/397 inline frontier API errors (Gemini rate-limited during",
        "  combined correction+groundback), zeroing all image-dependence metrics.",
        "",
        "New engineering change: `SGOCR_RG_LEAKY_LABEL_HARD_REJECT_ENABLED`",
        "Structurally rejects RG rows where anchor_label has color/shape tokens.",
        "Zero API cost. No error risk. Directly eliminates the identified leakage.",
        "",
        f"Cache: ita08_typed_gate intermediate. Workers: {perf['workers']}.",
        f"Groundback IoU base: {perf['groundback_iou_base']}  strict: {perf['groundback_iou_strict']}.",
        "",
        "## Engineering Changes",
        "",
        "| Location | Change |",
        "|---|---|",
        "| `semantic_dev40_tuning.py` | New field: `rg_leaky_label_hard_reject_enabled` + env var `SGOCR_RG_LEAKY_LABEL_HARD_REJECT_ENABLED` |",
        "| `full_pipeline_dev40.py` | New filter block after RG vdep check: reject RG rows with color/shape tokens in anchor_label |",
        "",
        "## Variant Summary",
        "",
...
def main() -> None:
    ...
    # All ita09 variants inherit the ita08_groundback-winning base:
    # typed frontier gate (DR/YN/TP/AP only, RG bypassed), qwen_degenerate_anchor_filter,
    # Nemotron v2 OCR, Qwen3-VL anchoring, structural fallback, anti-OCR prompt.
    ...
    # Flag sets for ita09 features
    groundback_base_flags = {
        "SGOCR_ANCHOR_LABEL_GROUNDBACK_ENABLED": "1",
        "SGOCR_ANCHOR_LABEL_GROUNDBACK_IOU_THRESHOLD": str(float(args.groundback_iou_base)),
    }
    groundback_strict_flags = {
        "SGOCR_ANCHOR_LABEL_GROUNDBACK_ENABLED": "1",
        "SGOCR_ANCHOR_LABEL_GROUNDBACK_IOU_THRESHOLD": str(float(args.groundback_iou_strict)),
    }
    leaky_reject_flags = {
    ...
    variants = {
        "ita09_gb_leaky_reject": {
            "env": {**common_env, **groundback_base_flags, **leaky_reject_flags},
            "note": VARIANT_DESCRIPTIONS["ita09_gb_leaky_reject"],
        },
        "ita09_gb_iou50": {
            "env": {**common_env, **groundback_strict_flags},
            "note": VARIANT_DESCRIPTIONS["ita09_gb_iou50"],
        },
        "ita09_leaky_pure": {
            "env": {**common_env, **leaky_reject_flags},
            "note": VARIANT_DESCRIPTIONS["ita09_leaky_pure"],
        },
        "ita09_full_push": {
            "env": {**common_env, **groundback_strict_flags, **leaky_reject_flags},
            "note": VARIANT_DESCRIPTIONS["ita09_full_push"],
        },
    }
    ...
    if existing_payload:
        ...
        payload = {
            ...
            "performance_profile": {
                ...
                "groundback_iou_base": float(args.groundback_iou_base),
                "groundback_iou_strict": float(args.groundback_iou_strict),
    ...
    for name, spec in variants.items():
        ...
        # Groundback variants must rebuild from the anchor stage (cache_level="ocr").
        # Non-groundback variants can reuse verified tuples (cache_level="verified").
        ...
        if result.get("status") in {"ok", "cached"}:
            ...
            if not args.skip_image_dependence and rows:
                ...
                _write_all(payload=payload, report_json=report_json, report_md=report_md, live_doc=live_doc)
                try:
                    idep_dir = out_dir / "evals" / "image_dependence_ita09"
