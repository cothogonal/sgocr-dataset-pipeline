...
# ita04 goal:
#   Baseline: ita03_structural_fallback (best ita03 config — ChartQA + structural fallback).
#   Test the upstream architecture shift: move centroid/anchor-quality gates before Gemini,
#   and wire vision-dependence + inline-frontier as empirical VERIFICATION GATES rather than
#   heuristic post-gen filters.
#
#   New flags in this sweep:
#     SGOCR_QWEN_ANTI_OCR_PROMPT_ENABLED          — add anti-OCR instruction to Qwen prompts
#     SGOCR_QWEN_DEGENERATE_ANCHOR_FILTER_ENABLED  — filter generic/OCR-text anchor labels pre-Gemini
#     SGOCR_UPSTREAM_CENTROID_FILTER_ENABLED        — centroid gate at select_candidates (pre-Gemini)
#     SGOCR_INLINE_FRONTIER_GATE_ENABLED            — reject rows where frontier grader says wrong
#     SGOCR_VISION_DEPENDENCE_GATE_ENABLED          — reject rows answerable without the image
#
#   Variants:
#     ita04_control            — ita03_structural_fallback baseline (no new flags)
#     ita04_anti_ocr           — Qwen anti-OCR prompt (prevents text-as-anchor labeling)
#     ita04_anchor_filter      — Degenerate anchor label filter (generic/OCR-text labels → skip)
#     ita04_centroid_up        — Centroid gate upstream in select_candidates (pre-Gemini)
#     ita04_frontier_gate      — Inline frontier gate: reject rows the grader gets wrong
#     ita04_vision_dep_gate    — Vision dependence gate: reject rows text-only can answer
#     ita04_upstream_bundle    — anti_ocr + anchor_filter + centroid_up (all upstream fixes)
#     ita04_gate_bundle        — frontier_gate + vision_dep_gate (both empirical verification gates)
#     ita04_all                — All ita04 features together
#
#   Cache strategy:
#     Qwen-modifying (anti-OCR changes prompt → different Qwen output): cache_level="ocr"
#       → ita04_anti_ocr, ita04_upstream_bundle, ita04_all
#     Everything else (post-Qwen filters/gates only): cache_level="verified"
#       → ita04_control, ita04_anchor_filter, ita04_centroid_up,
#          ita04_frontier_gate, ita04_vision_dep_gate, ita04_gate_bundle
#
#   NOTE (future work): naïve model baseline — run a cheap model (Haiku) on the same questions.
#   Gap between frontier and cheap model would be a proxy for dataset challenge ceiling.
#
#   NOTE (future work): LLM spatial judge for REVERSE_GROUND — instead of F1 ≥ 0.5 word overlap,
#   use a Gemini judge to score spatial accuracy given the image and anchor bounding box.

LIVE_DOC_DIR = REPO_ROOT / "tasks" / "mm_bridge" / "docs"
LIVE_DOC_NAME = "157_sgocr_mixed_ita04_launch_2026-04-12.md"
...
VARIANT_ORDER = [
    ...
    "ita04_centroid_up",
    "ita04_frontier_gate",
    "ita04_vision_dep_gate",
    "ita04_upstream_bundle",
    "ita04_gate_bundle",
    "ita04_all",
]

VARIANT_DESCRIPTIONS = {
    "ita04_control":         "ita03_structural_fallback baseline — no new upstream/gate flags",
    "ita04_anti_ocr":        "Qwen anti-OCR prompt: adds instruction not to copy visible text as anchor labels",
    "ita04_anchor_filter":   "Degenerate anchor filter: remove 'object part'/'surface'/OCR-text labels before Gemini",
    "ita04_centroid_up":     "Centroid gate moved upstream: filter near-center anchors at select_candidates (pre-Gemini)",
    "ita04_frontier_gate":   "Inline frontier gate: reject rows where Gemini Flash grader says answer is wrong",
    "ita04_vision_dep_gate": "Vision dependence gate: reject rows where text-only eval answers correctly (1 extra Flash call/row)",
    "ita04_upstream_bundle": "anti_ocr + anchor_filter + centroid_up — all upstream quality improvements",
    "ita04_gate_bundle":     "frontier_gate + vision_dep_gate — both empirical verification gates",
    "ita04_all":             "All ita04 features: upstream bundle + both gates",
}

# Variants that change the Qwen inventory prompt → must re-run Qwen (cache_level="ocr")
_QWEN_MODIFYING = {"ita04_anti_ocr", "ita04_upstream_bundle", "ita04_all"}


def parse_args() -> argparse.Namespace:
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ap = argparse.ArgumentParser(description="Run ita04: upstream architecture shift — gates before Gemini + empirical verification gates.")
    ap.add_argument("--bundle-id", default=f"sgocr_mixed_ita04_{stamp}")
    ap.add_argument("--source-name", default="chartqa50_textocr50_cocotext50_source_20260410_110952")
    ap.add_argument("--reference-report-json", default="sgocr_mixed_ita03_report.json")
    ...
    ap.add_argument(
        ...
        default="",
        help="Intermediate dir from a prior run for cache reuse. "
             "Qwen-modifying variants use cache_level=ocr; others use cache_level=verified.",
    )
...
def _render_live_doc(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Goal",
        "",
        "**Upstream architecture shift**: stop filtering bad rows post-generation and instead prevent",
        "bad inputs from reaching Gemini, then verify outputs empirically rather than heuristically.",
        "",
        "Baseline is `ita03_structural_fallback` (best ita03 config — fixes ChartQA with degenerate",
        "Qwen inventory detection + shape/region prompt).",
        "",
        "Three axes of improvement:",
        "1. **Qwen prompt hardening** (`ita04_anti_ocr`): instruction to label by visual object type,",
        "   never copy verbatim text. Targets REVERSE_GROUND vision leakage root cause.",
        "2. **Pre-Gemini anchor filtering** (`ita04_anchor_filter`, `ita04_centroid_up`):",
        "   degenerate labels and near-center anchors never reach Gemini generation.",
        "3. **Empirical verification gates** (`ita04_frontier_gate`, `ita04_vision_dep_gate`):",
        "   instead of heuristic filters, test the actual property we care about:",
        "   — *frontier gate*: does Gemini Flash get the answer right when shown the image?",
        "   — *vision dep gate*: does text-only eval fail? (one extra Flash call per row)",
        "",
        "**Note:** these gates introduce a small amount of natural, understandable error.",
        "Some genuinely good questions will fail the frontier gate due to model limitations.",
        "The vision dep gate is conservative: false negatives (vision-required questions that",
        "fail text-only) are kept; only clear leakage (text-only correct) is rejected.",
        "",
        "> **Future work (not implemented):** naïve model baseline — run a cheap model (Haiku)",
        "> on the same questions. Gap between frontier and cheap model = dataset challenge ceiling proxy.",
        "",
        "> **Future work (not implemented):** LLM spatial judge for REVERSE_GROUND — use a Gemini",
        "> judge to score spatial accuracy given image + anchor bounding box instead of F1 word overlap.",
        "",
        "## New Flags",
        "",
        "| Flag | Description |",
        "|---|---|",
        "| `SGOCR_QWEN_ANTI_OCR_PROMPT_ENABLED` | Add anti-OCR instruction to Qwen inventory prompts |",
        "| `SGOCR_QWEN_DEGENERATE_ANCHOR_FILTER_ENABLED` | Filter generic/'object part'/OCR-text anchor labels before Gemini |",
        "| `SGOCR_UPSTREAM_CENTROID_FILTER_ENABLED` | Filter near-center anchor candidates at select_candidates (pre-Gemini) |",
        "| `SGOCR_INLINE_FRONTIER_GATE_ENABLED` | Reject rows where inline frontier grader marks answer wrong |",
        "| `SGOCR_VISION_DEPENDENCE_GATE_ENABLED` | Reject rows where text-only eval answers correctly (1 extra Flash call/row) |",
        "",
        "## Sweep Parameters",
        "",
        f"| Parameter | Value |",
...
def main() -> None:
    ...
    common_env = {
        **baseline_env,
        ...
        # ita02_sceneaware baseline flags (kept through ita03 and ita04)
    ...
    centroid_up_flags = {
        "SGOCR_UPSTREAM_CENTROID_FILTER_ENABLED": "1",
    }
    frontier_gate_flags = {
        "SGOCR_INLINE_FRONTIER_GATE_ENABLED": "1",
    }
    vision_dep_gate_flags = {
        "SGOCR_VISION_DEPENDENCE_GATE_ENABLED": "1",
    }

    variants = {
        "ita04_control": {
            "env": {**common_env},
            "note": VARIANT_DESCRIPTIONS["ita04_control"],
        },
        "ita04_anti_ocr": {
            "env": {**common_env, **anti_ocr_flags},
            "note": VARIANT_DESCRIPTIONS["ita04_anti_ocr"],
        },
        "ita04_anchor_filter": {
            "env": {**common_env, **anchor_filter_flags},
            "note": VARIANT_DESCRIPTIONS["ita04_anchor_filter"],
        },
        "ita04_centroid_up": {
            "env": {**common_env, **centroid_up_flags},
            "note": VARIANT_DESCRIPTIONS["ita04_centroid_up"],
        },
        "ita04_frontier_gate": {
            "env": {**common_env, **frontier_gate_flags},
            "note": VARIANT_DESCRIPTIONS["ita04_frontier_gate"],
        },
        "ita04_vision_dep_gate": {
            "env": {**common_env, **vision_dep_gate_flags},
            "note": VARIANT_DESCRIPTIONS["ita04_vision_dep_gate"],
        },
        "ita04_upstream_bundle": {
            "env": {**common_env, **anti_ocr_flags, **anchor_filter_flags, **centroid_up_flags},
            "note": VARIANT_DESCRIPTIONS["ita04_upstream_bundle"],
        },
        "ita04_gate_bundle": {
            "env": {**common_env, **frontier_gate_flags, **vision_dep_gate_flags},
            "note": VARIANT_DESCRIPTIONS["ita04_gate_bundle"],
        },
        "ita04_all": {
            "env": {
                **common_env,
            ...
            "note": VARIANT_DESCRIPTIONS["ita04_all"],
        },
    }
    ...
    for name, spec in variants.items():
        ...
        # Cache strategy:
        # - Qwen-modifying variants (anti-OCR changes prompt): re-run Qwen but reuse Nemotron
        # - All others: skip GPU entirely, reuse verified cache from prior run
        ...
        if result.get("status") in {"ok", "cached"}:
            ...
            if not args.skip_image_dependence and rows:
                ...
                _write_all(payload=payload, report_json=report_json, report_md=report_md, live_doc=live_doc)
                try:
                    idep_dir = out_dir / "evals" / "image_dependence_ita04"
