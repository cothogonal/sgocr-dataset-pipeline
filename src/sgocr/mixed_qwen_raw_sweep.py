...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="Run an overnight mixed150 sweep for no-relabel Qwen local/raw anchor paths."
    )
    ap.add_argument("--bundle-id", default=f"sgocr_mixed_qwen_raw_{stamp}")
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Mixed Qwen Raw No-Relabel Sweep",
        "",
        ...
        "## Goal",
        "",
        "- Compare no-relabel Qwen local/raw anchor paths against a no-relabel Florence control.",
        "- Keep Nemotron OCR fixed and isolate anchor discovery/grounding behavior without Gemini relabel overwriting labels.",
        "",
        "## Performance Profile",
        "",
        ...
        "## Variants",
        "",
        "- `control_florence_selected_norelabel`: Florence local tag discovery + Qwen selected-tag grounding",
        "- `qwen_local_constrained_selected_norelabel`: constrained local Qwen crop discovery + Qwen selected-tag grounding",
        "- `qwen_local_open_selected_norelabel`: open/raw local Qwen crop discovery + Qwen selected-tag grounding",
        "- `qwen_local_constrained_global_inventory_norelabel`: constrained local Qwen crop discovery + one-shot constrained full-image inventory grounding",
        "",
        "## Reference Baselines",
        "",
    ...
    for name in (
        "control_florence_selected_norelabel",
        "qwen_local_constrained_selected_norelabel",
        "qwen_local_open_selected_norelabel",
        "qwen_local_constrained_global_inventory_norelabel",
    ):
...
def main() -> None:
    ...
    variants = {
        "control_florence_selected_norelabel": {
            ...
            "note": "control with relabel disabled",
        },
        "qwen_local_constrained_selected_norelabel": {
            "env": {
                **common_qwen_env,
            ...
            "note": "constrained local Qwen, relabel disabled",
        },
        "qwen_local_open_selected_norelabel": {
            "env": {
                **common_qwen_env,
            ...
            "note": "open/raw local Qwen, relabel disabled",
        },
        "qwen_local_constrained_global_inventory_norelabel": {
            "env": {
                **common_qwen_env,
            ...
            "note": "constrained local Qwen + one-shot inventory, relabel disabled",
        },
    }
